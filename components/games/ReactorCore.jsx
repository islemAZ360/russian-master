...
const BACKUP_DATA = [
    { russian: "Кот", arabic: "قطة" }, { russian: "Собака", arabic: "كلب" },
    { russian: "Дом", arabic: "منزل" }, { russian: "Мир", arabic: "سلام" }
];
...
export default function ReactorCore({ cards, onClose }) {
  ...
  const nextQuestion = () => {
    ...
    const wrongOptions = gameCards.filter(c => c.id !== target.id).sort(() => 0.5 - Math.random()).slice(0, 3);
    ...
    <div className="fixed inset-0 z-50 bg-[#050505] flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-lg">
            <div className="flex justify-between items-center mb-10">
                <div className="flex items-center gap-2 text-cyan-400 font-black tracking-widest"><IconRadioactive className="animate-spin-slow"/> REACTOR CORE</div>
            ...
            <div className="bg-cyan-900/10 border border-cyan-500/30 p-10 rounded-3xl text-center mb-6 backdrop-blur-md">
                <h1 className="text-5xl font-black text-white">{currentQuestion?.russian}</h1>
            </div>
            <div className="grid grid-cols-1 gap-3">
                {options.map((opt, i) => (
                    <button key={i} onClick={() => handleAnswer(opt)} className="p-4 bg-white/5 border border-white/10 rounded-xl text-lg font-bold text-white hover:bg-cyan-600 hover:border-cyan-400 transition-all font-cairo dir-rtl">
                        {opt.arabic}
                    </button>
                ))}
            </div>
