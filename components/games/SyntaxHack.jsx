...
import { IconCode, IconX, IconRefresh } from "@tabler/icons-react";
...
export default function SyntaxHack({ cards, onClose }) {
  ...
  const checkResult = () => {
      ...
      if (currentWord.toLowerCase() === currentCard.russian.toLowerCase()) {
          ...
          // اهتزاز
          const el = document.getElementById('syntax-box');
    ...
    <div className="fixed inset-0 z-50 bg-[#0a0a0a] flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-2xl">
            ...
            <div className="text-center mb-8">
                ...
                <h1 className="text-4xl font-bold text-white font-cairo">{currentCard?.arabic}</h1>
            </div>

            {/* منطقة الإجابة */}
            <div id="syntax-box" className="min-h-[80px] bg-white/5 border border-purple-500/30 rounded-2xl flex flex-wrap items-center justify-center gap-2 p-4 mb-8">
                <AnimatePresence>
                    {userAnswer.map(l => (
                        <motion.button
                            layoutId={l.id}
                            ...
                            className="w-12 h-12 bg-purple-600 rounded-lg font-bold text-xl text-white flex items-center justify-center shadow-lg"
                        >
                            {l.char}
                        </motion.button>
                    ))}
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-12">
                {shuffledLetters.map(l => (
                    <motion.button
                        ...
                        className="w-12 h-12 bg-gray-800 border border-white/10 rounded-lg font-bold text-xl text-gray-400 hover:bg-white/10 hover:text-white transition-colors"
                    >
                        {l.char}
                    </motion.button>
                ))}
            ...
            <button onClick={checkResult} className="w-full py-4 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl tracking-widest shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all">
                EXECUTE CODE
            </button>
        </div>
    </div>
