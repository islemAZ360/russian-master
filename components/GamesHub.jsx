...
// --- استيراد الألعاب الجديدة (البديلة) ---
import FlashProtocol from "./games/FlashProtocol"; // بديلة لـ Neural Breach
import ReactorCore from "./games/ReactorCore";     // بديلة لـ Memory Core
import SyntaxHack from "./games/SyntaxHack";       // بديلة لـ Data Rain

// --- استيراد الألعاب القديمة (التي تعمل ونريد الاحتفاظ بها) ---
...
export default function GamesHub({ cards }) {
  ...
  const GAMES = [
    // --- الألعاب الجديدة (بدائل للألعاب المعطلة) ---
    { 
        id: 'flash', title: 'FLASH PROTOCOL', desc: 'Speed binary decisions.', // بديلة Hacking
        color: 'text-yellow-500', bg: 'bg-yellow-500/10', icon: <IconBolt size={32}/> 
    },
    { 
        id: 'reactor', title: 'REACTOR CORE', desc: 'Multiple choice overload.', // بديلة Memory
        color: 'text-cyan-500', bg: 'bg-cyan-500/10', icon: <IconRadioactive size={32}/> 
    },
    { 
        id: 'syntax', title: 'SYNTAX HACK', desc: 'Reconstruct corrupted data.', // بديلة Rain
        color: 'text-purple-500', bg: 'bg-purple-500/10', icon: <IconCode size={32}/> 
    },

    // --- الألعاب القديمة (تم الاحتفاظ بها) ---
    { 
        id: 'scramble', title: 'CRYPTIC CIPHER', desc: 'Reconstruct data.', 
        color: 'text-blue-500', bg: 'bg-blue-500/10', icon: <IconAbc size={32}/> 
    },
    { 
        id: 'audio', title: 'AUDIO INTERCEPT', desc: 'Listen & decode.', 
        color: 'text-emerald-500', bg: 'bg-emerald-500/10', icon: <IconEar size={32}/> 
    },
    { 
        id: 'rapid', title: 'RAPID PROTOCOL', desc: 'True/False speed run.', 
        color: 'text-red-500', bg: 'bg-red-500/10', icon: <IconBolt size={32}/> 
    },
    { 
        id: 'logic', title: 'LOGIC GATE', desc: 'Multiple choice exam.', 
        color: 'text-orange-500', bg: 'bg-orange-500/10', icon: <IconPuzzle size={32}/> 
    },
  ];

  // --- توجيه الألعاب (Routing) ---
  
  // تشغيل الألعاب الجديدة
