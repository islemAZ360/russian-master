...
import { IconCpu, IconBrain, IconAbc, IconPlayerPlay, IconDeviceGamepad, IconEar, IconBolt, IconPuzzle, IconCloudRain } from "@tabler/icons-react";
...
export default function GamesHub({ cards }) {
  ...
  const GAMES = [
    { id: 'hacking', title: 'NEURAL BREACH', desc: 'Speed typing defense.', color: 'text-green-500', bg: 'bg-green-500/10', icon: <IconCpu size={32}/> },
    { id: 'memory', title: 'MEMORY CORE', desc: 'Sync data pairs.', color: 'text-purple-500', bg: 'bg-purple-500/10', icon: <IconBrain size={32}/> },
    { id: 'scramble', title: 'CRYPTIC CIPHER', desc: 'Reconstruct data.', color: 'text-blue-500', bg: 'bg-blue-500/10', icon: <IconAbc size={32}/> },
    { id: 'audio', title: 'AUDIO INTERCEPT', desc: 'Listen & decode.', color: 'text-yellow-500', bg: 'bg-yellow-500/10', icon: <IconEar size={32}/> },
    { id: 'rapid', title: 'RAPID PROTOCOL', desc: 'True/False speed run.', color: 'text-red-500', bg: 'bg-red-500/10', icon: <IconBolt size={32}/> },
    { id: 'logic', title: 'LOGIC GATE', desc: 'Multiple choice exam.', color: 'text-cyan-500', bg: 'bg-cyan-500/10', icon: <IconPuzzle size={32}/> },
    { id: 'rain', title: 'DATA RAIN', desc: 'Type falling words.', color: 'text-pink-500', bg: 'bg-pink-500/10', icon: <IconCloudRain size={32}/> },
  ];
  ...
  return (
    // تم إضافة pb-48 هنا لحل مشكلة الشريط السفلي
    <div className="w-full h-full flex flex-col p-6 overflow-y-auto custom-scrollbar pb-48">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-7xl mx-auto w-full z-10">
          {GAMES.map((game, i) => (
              <motion.div 
                key={game.id}
                ...
                className={`group relative h-48 cursor-pointer overflow-hidden rounded-3xl border border-white/10 bg-[#0a0a0a] hover:border-opacity-100 transition-all hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(0,0,0,0.5)]`}
              >
