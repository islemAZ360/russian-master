...
export default function StudyView({ 
  ...
  // تنظيف النصوص العائمة عند الخروج لتجنب أخطاء الذاكرة
  useEffect(() => {
    ...
    <div className="flex flex-col items-center justify-center h-full w-full relative pb-32">
        ...
        {currentCard ? (
            <div className="relative z-10 flex flex-col items-center w-full">
                <div className="mb-4 flex items-center gap-2 text-[var(--accent-color)] opacity-60"><IconInfinity size={20} className="animate-pulse" /><span className="text-[10px] font-mono tracking-[0.4em] uppercase">Neural Link Active</span></div>
                <StudyCard card={currentCard} sessionStats={sessionStats} onResult={handleResult} speak={speak} />
            </div>
        ) : (
            <div className="text-center p-12 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[3rem] shadow-2xl">
                <IconCpu size={80} className="text-[var(--accent-color)] mx-auto mb-6 animate-pulse" />
                <h2 className="text-4xl font-black text-[var(--text-main)] mb-2 tracking-tighter uppercase italic">Data Synced</h2>
                <button onClick={() => setCurrentView('home')} className="px-12 py-4 bg-[var(--accent-color)] text-white font-black rounded-2xl hover:opacity-90 shadow-lg transition-all uppercase tracking-widest text-xs mt-6">Return to Base</button>
            </div>
        )}
    </div>
  );
