...
import { 
  ...
  IconPlus, IconCategory, IconSettings, IconX, IconCheck, IconFolder, IconDeviceFloppy
...
const getCatTranslation = (catName, t) => {
    ...
    // تحويل النص (مثال: Daily Life -> cat_daily_life)
    const key = `cat_${catName.toLowerCase().replace(/ & /g, '_').replace(/ /g, '_')}`;
    const translated = t(key);
    // إذا لم يجد ترجمة للمفتاح، يعيد الاسم الأصلي كحل أخير
...
export function DataManager({ onAdd, onDelete, onUpdate, cards = [], isJunior }) {
  ...
  // استخراج المجموعات الفريدة بشكل مرتب
  ...
  // تصفير الصفحة عند البحث لضمان الأداء
  ...
  const handleRenameCategory = (oldName) => {
      ...
      // تحديث كافة السجلات التي تنتمي لهذه المجموعة
