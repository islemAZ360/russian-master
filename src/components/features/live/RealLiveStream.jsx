...
import { 
  IconBroadcast, IconLoader, IconDeviceTv, IconShieldCheck, IconWifi, IconLock 
} from "@tabler/icons-react";
import { useUI } from "@/context/UIContext";

export default function RealLiveStream() {
  const { startLive, liveState } = useUI(); // نستخدم السياق
  const [roomName, setRoomName] = useState("");
  const [status, setStatus] = useState("idle");
  const [logs, setLogs] = useState(["> SYSTEM_READY", "> WAITING_INPUT..."]);
  ...
  const handleJoin = () => {
    ...
    setTimeout(() => {
        ...
        // هنا السحر: نقوم بتشغيل البث في السياق العام
        startLive(roomName); 
    }, 1500);
  };

  // إذا كان البث يعمل بالفعل، نعرض رسالة أننا متصلين
  if (liveState.isActive) {
      return (
        <div className="flex flex-col items-center justify-center h-full w-full text-center">
            ...
            <h2 className="text-3xl font-black text-white tracking-tighter mb-2">SIGNAL ACTIVE</h2>
            <p className="text-white/40 font-mono text-sm mb-8">SECURE CHANNEL: {liveState.roomName}</p>
            <div className="text-xs text-cyan-500/50 font-mono border border-cyan-500/20 px-4 py-2 rounded">
                YOU CAN NAVIGATE FREELY. CONNECTION WILL MINIMIZE automatically.
            </div>
        </div>
      );
  }
  ...
  return (
    <div className="w-full h-full flex flex-col relative font-sans overflow-hidden items-center justify-center">
        <motion.div 
            ...
            <div className="space-y-6 relative z-10">
                <div className="relative group/input">
                    ...
                    <input 
                        ...
                        className="w-full bg-black/50 border border-white/10 rounded-xl py-4 pl-12 pr-4 text-white outline-none focus:border-cyan-500 focus:bg-cyan-950/10 transition-all font-mono text-sm uppercase tracking-widest"
                ...
                >
                    ...
                    <span className="tracking-widest text-xs">{status === "scanning" ? "ENCRYPTING..." : "INITIALIZE UPLINK"}</span>
                </button>
                
                {/* Console Logs */}
                <div className="bg-black/40 rounded-lg p-3 h-20 overflow-hidden font-mono text-[9px] text-green-500/60 border border-white/5 flex flex-col justify-end">
