...
/**
 * مكون StatCard الداخلي - تم تعريفه هنا لضمان عدم حدوث ReferenceError
 */
...
export default function AdminDashboard({ currentUser }) {
  ...
  // --- حالات الميزات الخاصة (الدعم والبث) ---
  const [broadcastMsg, setBroadcastMsg] = useState("");
  const [selectedTicket, setSelectedTicket] = useState(null);
  const [replyText, setReplyText] = useState("");
  
  // --- حالات رقابة المحادثات المباشرة ---
  ...
  // --- وظائف التحكم الإداري (Actions) ---

  // وظيفة المبادرة ببدء محادثة مع أي مستخدم (حتى لو لم يتواصل هو أولاً)
  const initiateChatWithUser = (targetUser) => {
      ...
      if (existingTicket) {
          ...
          // إنشاء تذكرة افتراضية في الواجهة (Virtual Buffer)
          setSelectedTicket({
              id: targetUser.id,
  ...
  // إرسال رد الدعم (أو بدء المحادثة فعلياً)
  const sendSupportReply = async () => {
      ...
      try {
          ...
          if (selectedTicket.isVirtual) {
              // تسجيل المحادثة في Firestore لأول مرة
              ...
              await addDoc(collection(db, "notifications"), {
                  userId: targetUserId,
                  title: "URGENT: COMMAND CENTER UPLINK",
                  message: "An admin has initiated a secure communication channel with you.",
                  type: "support_reply",
    ...
    <div className="fixed inset-0 z-[200] flex bg-[#050505] text-white font-sans overflow-hidden" dir={dir}>
        ...
        <nav className={`fixed md:relative top-0 left-0 h-full w-72 bg-black border-r border-white/10 z-50 transition-transform duration-300 md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : (dir === 'rtl' ? 'translate-x-full' : '-translate-x-full')}`}>
            <div className="p-8 h-24 border-b border-white/10 flex items-center gap-4">
                ...
                <div>
                    <h1 className="font-black text-xl uppercase tracking-tighter leading-none">NEXUS<span className="text-indigo-500">OS</span></h1>
                    <span className="text-[9px] text-gray-600 font-mono tracking-widest uppercase">Admin Kernel v4.2</span>
                </div>
            </div>
        ...
        <main className="flex-1 flex flex-col h-full bg-[#050505] relative overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden relative">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {users.map(u => (
                                        <tr key={u.id} className="hover:bg-white/5 transition-all group">
                                            ...
                                            <td className="p-6">
                                                {u.isBanned ? (
                                                    <span className="text-[9px] bg-red-950/40 text-red-500 px-3 py-1 rounded-full border border-red-500/30 font-black animate-pulse">Terminated</span>
                                                ) : (
                                            ...
                                            <td className="p-6 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <button onClick={() => initiateChatWithUser(u)} className="p-2.5 rounded-xl bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white transition-all shadow-lg"><IconMessagePlus size={18}/></button>
                                                    <button onClick={() => toggleBan(u.id, u.isBanned)} className={`px-4 py-2 rounded-xl font-black text-[10px] uppercase transition-all shadow-lg ${u.isBanned ? 'bg-emerald-600 text-white' : 'bg-red-900/30 text-red-500 border border-red-500/20 hover:bg-red-600 hover:text-white'}`}>{u.isBanned ? t('admin_unban') : t('admin_ban')}</button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                ...
                {/* 3. رقابة المحادثات (Orbital Supervision) */}
                            ...
                            {selectedTicket ? (
                                <>
                                    <div className="p-6 bg-zinc-900/50 border-b border-white/5 flex justify-between items-center shrink-0">
                                        ...
                                        {!selectedTicket.isVirtual && <button onClick={() => markTicketResolved(selectedTicket.id)} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase transition-all shadow-lg shadow-emerald-900/20">Resolve Signal</button>}
                                    </div>
                ...
                {activeView === 'broadcast' && (
                    <div className="h-full flex items-center justify-center animate-in zoom-in duration-500 p-0">
                        <div className="max-w-3xl w-full bg-[#0c0c0c] border border-white/10 p-12 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                            ...
                            <textarea value={broadcastMsg} onChange={e=>setBroadcastMsg(e.target.value)} className="w-full h-56 bg-black border border-zinc-800 rounded-3xl p-8 text-white text-xl font-mono focus:border-red-600 outline-none transition-all resize-none mb-8 shadow-inner" placeholder="TYPE SYSTEM ALERT MESSAGE HERE..." dir="auto"/>
