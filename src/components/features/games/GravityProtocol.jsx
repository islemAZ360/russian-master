...
export default function GravityProtocol({ onClose }) {
  ...
  // إعداد الكرات
  useEffect(() => {
    // نختار الكلمات بدون تكرار
    const uniqueOptions = [...new Set(currentData.options)]; 
    
    const orbs = uniqueOptions.map((word, i) => {
        // نحدد الموقع النهائي عشوائياً
        const randomLeft = (i * (90 / uniqueOptions.length)) + 5; // نسبة مئوية من العرض
        const randomBottom = 20 + Math.random() * 150; // بكسل من الأسفل (أرضية عريضة)

        return {
            id: `orb-${level}-${i}`,
            text: word,
            left: randomLeft,   // الموقع الأفقي الثابت
            bottom: randomBottom, // الموقع العمودي الثابت
            delay: i * 0.15     // تأخير السقوط
        };
    });
    setFloorOrbs(orbs);
    setIsBoxOpen(false);
  ...
  // عند بدء السحب: نفتح الصندوق
  const handleDragStart = () => {
      setIsBoxOpen(true);
  };

  // عند الإفلات: نغلق الصندوق ونتحقق من الإجابة
  const handleDragEnd = (event, info, id, text) => {
    setIsBoxOpen(false);
    
    // نحدد موقع الماوس/الإصبع عند الإفلات
    const dropPoint = { 
        x: info.point.x, 
        y: info.point.y 
    };
    ...
    // هل تم الإفلات داخل حدود الصندوق؟
    const isInsideBox = (
    ...
    <div ref={containerRef} className="fixed inset-0 z-[200] bg-[#050505] text-cyan-500 font-mono overflow-hidden select-none">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
          <motion.div 
            ref={boxRef}
            animate={{ 
                scale: isBoxOpen ? 1.1 : 1, // تكبير الصندوق عند السحب
          ...
          >
              {/* Lid Animation (باب الصندوق) */}
              <motion.div 
                initial={{ y: "0%" }}
                animate={{ y: isBoxOpen ? "-100%" : "0%" }} // يفتح للأعلى عند السحب
      ...
      {/* Floor Orbs (الفقاعات القابلة للسحب) */}
      <div className="absolute inset-0 z-30 pointer-events-none">
          {floorOrbs.map((orb) => (
              <motion.div
                ...
                // خصائص السحب
                drag
                dragConstraints={containerRef} // حدود الشاشة
                dragElastic={0.2}
                dragSnapToOrigin={true} // تعود لمكانها إذا أفلتها في الفراغ
                dragMomentum={false}    // إيقاف الزحلقة لسهولة التحكم
                
                onDragStart={handleDragStart}
                onDragEnd={(e, i) => handleDragEnd(e, i, orb.id, orb.text)}
                
                // عند السحب: تكبير + رفع الطبقة (z-index) لتكون فوق كل شيء
                whileDrag={{ scale: 1.2, zIndex: 9999, cursor: 'grabbing' }}
                whileHover={{ scale: 1.1, zIndex: 50, cursor: 'grab' }}

                // أنيميشن السقوط الأولي (من الأعلى إلى الموقع المحدد)
                ...
                // تحديد الموقع الثابت (CSS) لكي لا يتعارض مع Framer Motion drag
                style={{ 
                    position: 'absolute',
                    left: `${orb.left}%`,
                    bottom: `${orb.bottom}px`,
                    touchAction: "none", // مهم جداً لمنع التمرير في الموبايل أثناء السحب
                    pointerEvents: 'auto' // تفعيل النقر
                }}
      ...
      {/* Feedback Message (رسالة النجاح/الفشل) */}
      <AnimatePresence>
        {feedback && (
            <motion.div 
                // وضعنا التوسيط هنا في خصائص الحركة لإجبار Framer Motion على الالتزام به
